...
public class LockAnnotationAspect {
    ...
    public void before(JoinPoint joinPoint) {
        ...
        if (Boolean.FALSE.equals(tryLock))
            throw new LockRuntimeException(String.format("alias:%s key:%s 已存在锁！", locking.alias(), Arrays.toString(locking.keys())));
    }

    @After("pointCut()")
    public void after(JoinPoint joinPoint) {
        log.debug("LockAnnotationAspect 后置通知:" + joinPoint);
        MethodSignature methodSignature = (MethodSignature) joinPoint.getSignature();
        Locking locking = getLocking(methodSignature);
